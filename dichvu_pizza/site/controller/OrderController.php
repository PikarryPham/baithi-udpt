...
class OrderController
{
    ...
    function edit()
    {
        ...
        if ($MaDH) {
            ...
            if ($order != null) {
                ...
                $_SESSION['error'] = "Đơn hàng mã: $MaDH bạn tìm kiếm không tồn tại";
            }
        } else {
            $orders = $orderRepository->getAll();
        }
    ...
    function edit_order()
    {
        ...
        if ($MaDH) {
            ...
            $order = $orderRepository->getByCond($cond);
            if ($order != null) {
                $services_order = $serviceRepository->find($order->MaDV);
            }else{
                $_SESSION['error'] = "Đơn hàng mã: $MaDH bạn tìm kiếm không tồn tại";
            }
        }
        require './site/view/edit_order.php';
    }
    ...
    function destroy()
    {
        if (isset($_GET['MaDH'])) {
            ...
            if ($TrangThai == 'DAKHOITAO') {
                $run_order = $orderRepository->destroy($MaDH);
                global $conn;
                $sql = "UPDATE `order` SET `Status`='HUY' WHERE `OrderID` = $MaDH";
